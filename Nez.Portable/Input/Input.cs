...
namespace Nez
{
	public static class Input
	{
        ...
        public static Vector2 MousePositionInGrid()
        {
            //TODO If map grid placement is weird at any point this might be why hardcoded tilesize.
            Vector2 result = Core.scene.camera.mouseToWorldPoint();
            result.X = (int)(result.X / 16) * 16;
            result.Y = (int)(result.Y / 16) * 16;
            return result;
        }

        public static Vector2 MousePositionInGridTile()
        {
            Vector2 result = Core.scene.camera.mouseToWorldPoint();
            result.X = (int)(result.X / 16);
            result.Y = (int)(result.Y / 16);
